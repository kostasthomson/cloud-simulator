...
class PSwitch:
    ...
    def __init__(self, start: int = 0, end: int = 0, resource_type: int = 0,
                 vrms_list: Optional[List[List['VRM']]] = None,
                 poll_interval_pswitch: float = 0.0, c: float = 0.0, p: float = 0.0,
        ...
        self.poll_interval_pswitch: float = poll_interval_pswitch
        self.vrms: List['VRM'] = []
        ...
        if vrms_list is not None and end > start:
            self._initialize(start, end, resource_type, vrms_list)

    def _initialize(self, start: int, end: int, resource_type: int,
                    vrms_list: List[List['VRM']]) -> None:
        self.alloc = 1
        self.number_of_vrms = end - start

        for i in range(start, end):
            self.vrms.append(vrms_list[resource_type][i])
    ...
    def update_state_info(self, tstep: float) -> None:
        if self.alloc:
            if int(tstep) % int(self.poll_interval_pswitch) == 0 if self.poll_interval_pswitch > 0 else True:
                ...
                for i, vrm in enumerate(self.vrms):
                    vrm_spmsa = vrm.get_spmsa()
                    self.available_processors[i] = vrm_spmsa[0]
                    self.total_processors[i] = vrm_spmsa[1]
                    self.available_memory[i] = vrm_spmsa[2]
                    self.total_memory[i] = vrm_spmsa[3]
                    self.available_storage[i] = vrm_spmsa[4]
                    self.total_storage[i] = vrm_spmsa[5]
                    self.available_accelerators[i] = vrm_spmsa[6]
                    self.total_accelerators[i] = vrm_spmsa[7]
    ...
    def deploy(self, resources: List[List['Resource']], network: 'Network',
        ...
        if choice == -1:
            ...
            for i in range(choice):
                rem_proc, rem_mem, rem_sto, rem_acc = self.vrms[i].obtain_resources(
                    pores, rem_proc, rem_mem, rem_sto, rem_acc)
                    ...
                    if self.spmsa[7] > 0:
                        for j in range(4):
                            ssum += self.ws[j] * self.deassessment_functions(
                                ...
                                self.total_accelerators[i],
                                -float(len(pores)) * pores[0].get_total_memory(),
                                self.total_memory[i], j)
                    else:
                        for j in range(4):
                            ssum += self.ws[j] * self.deassessment_functions(
                                -float(len(pores)) * pores[0].get_total_processors(),
                                self.total_processors[i],
                                -float(len(pores)) * pores[0].get_total_memory(),
                                self.total_memory[i], j)
                ...
                if rem_proc <= 0.0 and rem_mem <= 0.0 and rem_sto <= 0.0 and rem_acc <= 0.0:
                    break

            if not (rem_proc <= 0.0 and rem_mem <= 0.0 and rem_sto <= 0.0 and rem_acc <= 0.0):
                ...
                if self.spmsa[7] > 0:
                    for j in range(4):
                        ssum += self.ws[j] * self.deassessment_functions(
                            ...
                            self.total_accelerators[choice],
                            float(len(ores)) * ores[0].get_total_memory(),
                            self.total_memory[choice], j)
                else:
                    for j in range(4):
                        ssum += self.ws[j] * self.deassessment_functions(
                            float(len(ores)) * ores[0].get_total_processors(),
                            self.total_processors[choice],
                            float(len(ores)) * ores[0].get_total_memory(),
                            self.total_memory[choice], j)
        ...
        if self.spmsa[7] > 0:
            for i in range(4):
                ssum += self.ws[i] * self.deassessment_functions(
                    -req_acc, self.spmsa[7], -req_mem, self.spmsa[3], i)
        else:
            for i in range(4):
                ssum += self.ws[i] * self.deassessment_functions(
